...
export async function getServerUsage(serverID) {
    ...
    let cpuPercent = Math.round(resourcesData.resources.cpu_absolute / serverData.limits.cpu * 1000) / 10
    ...
    let memoryPercent = Math.round((memoryUsed / serverData.limits.memory) * 1000) / 10

    // Converts disk usage in bits to mebibytes
    let diskUsed = Math.round((resourcesData.resources.disk_bytes / 1024) / 1048.576)
    let diskPercent = Math.round((diskUsed / serverData.limits.disk) * 1000) / 10
    ...
    let uptimeMinutes = Math.round((resourcesData.resources.uptime / 1000 / 60) % 60)
